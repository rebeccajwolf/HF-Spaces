...
from typing import Union, List, Literal, Final, NamedTuple, Type
import copy
...
def browserSetupv3(isMobile: bool = False, proxy: str = None) -> WebDriver:
    ...
    if ARGS.edge:
        ...
        options = uc.ChromeOptions()
    options.add_argument("--user-agent=" + user_agent[0])
    options.add_argument('--lang=' + LANG.split("-")[0])
    prefs = {"profile.default_content_setting_values.geolocation" :2,
                    "profile.default_content_setting_values.notifications": 2,
                    "credentials_enable_service": False,
...
def checkBingLogin(browser: WebDriver, isMobile: bool = False):
    ...
    if not isElementExists(browser, By.XPATH, '//*[@id="id_s" and @aria-hidden="true"]' if not isMobile else '//*[@id="hb_n" and @data-clarity-mask="true"]'):
        while True:
            goToURL(browser, 'https://www.bing.com/fd/auth/signin?action=interactive&provider=windows_live_id&return_url=https%3A%2F%2Fwww.bing.com%2F')
            ...
            if isMobile:
                ...
                time.sleep(calculateSleep(7))
            try:
                if isElementExists(browser, By.XPATH, '//*[@id="id_s" and @aria-hidden="true"]' if not isMobile else '//*[@id="hb_n" and @data-clarity-mask="true"]'):
                    if isMobile:
                        browser.find_element(By.XPATH, '//*[@id="HBFlyoutClose"]').click()
                        time.sleep(2)
                    return
...
def createMessage():
    ...
    total_earned = 0
    total_overall = 0
    message = f'*{BOT_NAME}*\n\n📅 Daily report {today}\n\n' if ARGS.telegram else f'**{BOT_NAME}**\n\n📅 Daily report {today}\n\n'
...
def sendToTelegram(message):
    """send to telegram"""
    apprise = Apprise()
    apprise.add(os.environ.get("TOKEN", None))
    if len(message) > 4096:
        messages = [message[i:i+4096] for i in range(0, len(message), 4096)]
        for ms in messages:
            apprise.notify(body=ms, body_format=NotifyFormat.MARKDOWN)
    else:
        apprise.notify(body=message, body_format=NotifyFormat.MARKDOWN)


def sendToDiscord(message):
    """send to discord"""
    webhook_url = os.environ.get("TOKEN", None)
    if len(message) > 2000:
